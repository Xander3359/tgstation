...
/// normal put_in_hands() drops the item ontop of the player, this drops it at the suit's loc
/obj/item/robot_suit/proc/put_in_hand_or_drop(mob/living/user, obj/item/I)
...
/obj/item/robot_suit/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	if(istype(tool, /obj/item/mmi))
		...
		if(is_banned_from(brainmob.ckey, JOB_CYBORG) || QDELETED(src) || QDELETED(brainmob) || QDELETED(user) || QDELETED(inserting_mmi) || !Adjacent(user))
